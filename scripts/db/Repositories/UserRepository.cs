...
/// </summary>
public class UserRepository : ISingleton
{
    /// <summary>
    /// 사용자 ID로 알림 설정을 조회합니다.
    /// </summary>
    /// <returns>없으면 null</returns>
    public async Task<UserSettingEntity?> GetUserSettingAsync(ulong userId, MySqlConnection connection, MySqlTransaction? transaction = null)
    {
...
",
    ...
    public async Task<List<StartAlertEntity>> GetAlertUsers(MySqlConnection connection, MySqlTransaction? transaction = null)
    {
    ...
    public async Task<bool> InsertAlertParty(string PARTY_KEY, MySqlConnection connection, MySqlTransaction? transaction = null)
    {
        var sql = @"
...
";
    ...
    public async Task<bool> RemoveAlertParty(string PARTY_KEY, MySqlConnection connection, MySqlTransaction? transaction = null)
    {
        var sql = @"
...
";
        var affectedRows = await connection.ExecuteAsync(sql, new { PARTY_KEY }, transaction: transaction);
        return affectedRows > 0;
    }
    ...
    /// </summary>
    public async Task<bool> SetUserSettingAsync(UserSettingEntity entity, MySqlConnection connection, MySqlTransaction? transaction = null)
    {
        var sql = @"
